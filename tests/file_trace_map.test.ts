...
describe('FTM initialization and IO', () => {
    ...
    test('Open a File Trace Map', async () => {
        ...
        expect(ftm?.indexBlocks[0].length).toBe(1)
        expect(ftm?.indexBlocks[0].get(0)).toBe(66)
        expect(ftm?.indexBlocks[0].get(1)).toBe(undefined)
        expect(ftm?.indexBlocks[0].next).toBe(0)
